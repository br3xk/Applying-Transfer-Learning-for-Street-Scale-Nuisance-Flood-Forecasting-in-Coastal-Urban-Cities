...
"""
4-Hour LSTM Forecasting Base Model for Street-Scale Flooding in Norfolk, VA
...
To run with different base model configurations (varying % of source streets):

** 10% base model [Ss=18] **
trial_data ='S_S0.1_E1.0'
FID_selected=pd.read_csv(path_FIDs+"Source_10.csv")

** 20% base model [Ss=36] **
trial_data ='S_S0.2_E1.0' 
FID_selected=pd.read_csv(path_FIDs+"Source_20.csv")

** 60% base model [Ss=108] ** 
trial_data ='S_S0.6_E1.0'
FID_selected=pd.read_csv(path_FIDs+"Source_60.csv")

** 100% base model [Ss=180] **
trial_data ='S_S1.0_E1.0' 
FID_selected=pd.read_csv(path_FIDs+"Source_100.csv")
...
train_Events=[1,2,3,4,5,7,8,9,10,11,15,16,19,20,21,22,24,26,28,29,30,32,34,35,36,39,41,42,43,44] #fixed
validation_Events=[6,17,23,25,27,31,37]
test_Events=[12,13,14,18,33,38,40]

#specify FIDs for base model
path_FIDs="input/"

#set of streets data
FID_selected=pd.read_csv(path_FIDs+"Source_10.csv") #change
...
for bootstrap_index in range(num_bootstraps):
    ...
    #callback
    mc_filename = f'best_model_base_bootstrap_{bootstrap_index + 1}.h5'
    ...
    #time
    end_base = time.time()
    time_base = (end_base - start_base)/60
    print(f"{hp_model}_{bootstrap_index + 1}_pred_time: ", time_base)
    time_df_base = pd.DataFrame({'Time': [time_base]})
    time_df_base.to_csv(f"1.Result_TF/time_{hp_model}_{bootstrap_index + 1}.csv") 
    times.append(time_base)
    
    #epoch
    epochs_base = len(history.epoch)
    print(f"{hp_model}_{bootstrap_index + 1}_epochs: ", epochs_base)
    epochs_df_base = pd.DataFrame({'Epoch': [epochs_base]})
    epochs_df_base.to_csv(f"1.Result_TF/epoch_{hp_model}_{bootstrap_index + 1}.csv") 
    epochs.append(epochs_base)
    ...
    history_df_base=pd.DataFrame(history.history)
    history_df_base.to_csv(f"1.Result_TF/history_{hp_model}_{bootstrap_index + 1}.csv")
...
mse_scores_df_base = pd.DataFrame({'MSE': [mse_scores]})
rmse_scores_df_base = pd.DataFrame({'RMSE': [rmse_scores]})
rmses_df_base = pd.DataFrame({'RMSE': [rmses]})
epochs_df_base = pd.DataFrame({'Epoch': [epochs]})
times_df_base = pd.DataFrame({'Time': [times]})
    
mse_scores_df_base.to_csv(f"1.Result_TF/mse_scores_{hp_model}_{trial_all}.csv") 
rmse_scores_df_base.to_csv(f"1.Result_TF/rmse_scores_{hp_model}_{trial_all}.csv") 
rmses_df_base.to_csv(f"1.Result_TF/rmses_{hp_model}_{trial_all}.csv") 
epochs_df_base.to_csv(f"1.Result_TF/epochs_{hp_model}_{trial_all}.csv") 
times_df_base.to_csv(f"1.Result_TF/times_{hp_model}_{trial_all}.csv") 
...
models[best_model_index].save("best_source_model.h5")
models[best_model_index].save(f"1.Result_TF/{hp_model}_best_BTS5.h5")
print("Saved best source model to disk")
...
print("test RMSE for base BTS5:", rmse_ensemble)
